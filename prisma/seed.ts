...
async function main() {
  ...
  for (let i = 0; i < totalEvents; i++) {
    const category = savedCategories[faker.number.int({ min: 0, max: savedCategories.length - 1 })];
    ...
    const eventDate = faker.date.between({ from: startOfMonth, to: endOfMonth });
    ...
    const event = await prisma.event.create({
      data: {
        ...
        quota: faker.number.int({ min: 20, max: 50 }),
